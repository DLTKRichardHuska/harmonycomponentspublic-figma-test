...
/** Mirrors reference `src/data/navigation.ts`. */
export const demoNavigation: NavSection[] = [
  {
    title: 'Getting Started',
    items: [{ title: 'Getting Started', href: '/getting-started', icon: 'clipboard-document-check' }],
  },
  {
    title: 'Changelog',
...
/** Exported UI component count from reference catalog. */
...
export function isNavItemActive(itemHref: string, pathname: string): boolean {
  ...
  if (href === '/components/kanban') {
    return path === '/components/kanban' || path === '/cp/kanban';
  }
  return path === href;
}

export function resolveKanbanHref(product: string): string {
  return product === 'cp' ? '/cp/kanban' : '/components/kanban';
}
