...
const outDir = join(__dirname, '..', 'card-2');
  ...
  return page.evaluate(({ lab, isRef }) => {
    ...
    const styleOf = (el) => {
      ...
      return {
        ...
        bodyFontWeight: bodyP ? getComputedStyle(bodyP).fontWeight : null,
        bodyColor: bodyP ? getComputedStyle(bodyP).color : null,
        bodyLineHeight: bodyP ? getComputedStyle(bodyP).lineHeight : null,
        bodyText: body ? (body.textContent||'').trim().slice(0,100) : null,
        iconCount: icons.length,
        iconSizes: icons.slice(0, 3).map((btn) => {
