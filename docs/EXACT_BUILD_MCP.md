...
- **Response shape:** Return **one** build payload and **guidance** in one response so the AI has a single authoritative blob (DOM order, content, and styles). Include root-level build-critical fields, not only `specs[key]`:
  - `buildSpec` – The canonical spec object from `specs[key]` (props, layout, spacing, dimensions, typography, borders, states, icons, and when applicable **template**, **structure**). All values are resolved (no null, no transparent, no var()). Template items that use a named icon should include **iconPath** so the consumer can render the icon without a separate icon-mappings lookup. Apply these values exactly; no deviations.
...
- **Icons:** For any icon referenced by name (e.g. in defaultContent or template), the builder must use the SVG from **payload.icons[name]** when present. Do not redraw or substitute a different SVG; use the exact markup or asset from the spec.
