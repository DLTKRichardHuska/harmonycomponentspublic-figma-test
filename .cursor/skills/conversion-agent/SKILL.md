...
3. **Only if the gate passes** — **AskQuestion — element strategy (mandatory):** choices come from the converter’s `elementStrategies.allowed` (e.g. react-mui: Existing / Custom / Skip; shadcn: Component / Skip; vanilla: `native` / `hybrid` / `web-component` / `skip`; figma: `figma-component` / `figma-variable` / `skip`). Cannot write plan without answers.
