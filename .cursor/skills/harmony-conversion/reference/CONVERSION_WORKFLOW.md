...
| **Ready to release** | Run `validate_release_readiness.mjs --release-version <semver>` (train labels on `main` are expected); user tags `vX.Y.Z`; publish job rewrites versions in the workspace and publishes |
